...
export = function loginSteps() {
    ...
    this.When(/^he enters a great credentials$/, function () {
        return this.stage.theActorInTheSpotlight().attemptsTo(
            AuthenticateWith.item({ email:'client@transportsystems.co', password:'Ts12345*' }).on('/v1/login')
        )
    });
    ...
    this.When(/^user enters (.*) and (.*)$/, function (name: String, directory: String) {
        return this.stage.theActorInTheSpotlight().attemptsTo(
            Post.item({ directory: directory, type: "SIMPLE_VOLUMES", name: name }).on('/v1/analyses')
        )
    });

    /*this.When(/^user enters data to create the analysis$/, function (table: any) {
        return this.stage.theActorInTheSpotlight().attemptsTo(
            //Post.item({ directory: table., type: "SIMPLE_VOLUMES", name: name }).on('/v1/analyses')
        )
    });*/

    this.When(/^user enters data to create the analysis with data in excel at '\/home\/abraham\/Documents\/allanalysis\.csv'$/, function () {
    ...
    this.Given(/^he creates the analysis$/, function () {
        return this.stage.theActorInTheSpotlight().attemptsTo(
            Post.item({ directory: 'regressiontest', type: "SIMPLE_VOLUMES", name: 'name' }).on('/v1/analyses')
        )
    });

    this.Given(/^he creates the movement$/, function () {
        
        return this.stage.theActorInTheSpotlight().attemptsTo(
            Post.item({ cameraDirectory: "Camara1", entryLine: [[0.18, 0.48], [0.54, 0.91]], exitLine: [[0.26, 0.42], [0.61, 0.86]], name: "Uno", snapshotIndex:0 }).on('/v1/analyses/10/movements')
        )
    });

    this.Then(/^he should see the new analysis$/, function () {
        return this.stage.theActorInTheSpotlight().attemptsTo(
            See.if(LastResponse(),hasResponseCode(200))
            )
      });
